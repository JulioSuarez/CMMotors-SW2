    ...
    <div class="invoice-box">
        ...
        <table class="items-table mt" cellpadding="0" cellspacing="0">
            ...
            @for ($i = 0; $i < $l; $i++)
                <tr class="item">
                    ...
                    <td class="hidden">
                        {{ $factura += $productos[$i]->precio_venta_sin_factura * $productos[$i]->cantidad }}</td>
                    <td class="hidden">
                        {{ $sinfactura += $productos[$i]->precio_venta_con_factura * $productos[$i]->cantidad }}</td>
                    <td class="hidden">{{ $stock += $productos[$i]->cantidad }}</td>
                    <td class="hidden">
                        {{ $utilidad += ($productos[$i]->precio_venta_con_factura - $productos[$i]->precio_compra) * $productos[$i]->cantidad }}
                    </td>
                </tr>
            @endfor
        </table>
        <table class="">
            <thead>
                ...
                <tr class="item">
                    <th class="text-right">
                        Sumatoria de los precios de venta:
                    </th>
                    <td>
                        {{ $factura }}
                    </td>
